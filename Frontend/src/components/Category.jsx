        ...
        <div className="flex justify-around overflow-x-auto scrollbar-hide">
          {categories.map((cat, index) => (
            <div
              ...
              className="flex items-center gap-2 px-3 py-1 shrink-0 cursor-pointer opacity-70 hover:opacity-100 transition whitespace-nowrap"uuuuuuuuuuu
            >
              {cat.icon}
      ...
      {showAllCategories && (
        <div className="absolute top-[100%] left-0 w-full bg-white border-t border-gray-300 shadow-lg z-20 p-4 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
          {allCategories.map((cat, index) => (
            <div key={index} className="text-sm font-medium cursor-pointer hover:text-purple-600 transition">
              {cat}
            </div>
          ))}
        </div>
